...
if ($action == 'update') {
    ...
    if (empty($$pt_id)) {
        ...
        if (!$a->error) {
            ...
            if ($res->count) {
                ...
                st_do_action('add_user_log',"Created regular invoice #{$invoiceNumber}");
            } else {
    ...
    if (!$a->error && !empty($$pt_id)) {
        foreach ($itemName as $k => $v) {
            ...
            if (!$a->error) {
	            ...
	            $__itemName = addslashes($itemName[$k]);
    ...
    if (!$a->error) {
        foreach ($_itemName as $k => $v) {
            ...
            if (!$a->error) {
	            ...
	            $__itemName = addslashes($_itemName[$k]);
    ...
    if (isset($_REQUEST['sendInvoice']) && !$a->error) {
        ...
        if ($settings->attach_pdf_invoice == 'y') {
	        ...
	        $res = $a->sendMail($customerEmail, "New invoice #{$_invoiceNumber} to pay", "send_invoice.html", $mailData, false, $send_cc, $send_bcc);
        }
        if($res=== true) {
        ...
        if(!empty($send_cc)){ $invoice->addHistory('send',"Invoice sent to '{$send_cc}' CC");}
        if(!empty($send_bcc)){ $invoice->addHistory('send',"Invoice sent to '{$send_bcc}' BCC");}
    }
...
$itemsArray = $itemModel->getItemsListForTypeahead();
...
if(isset($settings->invoice_terms_preset) && !empty($settings->invoice_terms_preset)){
    $invoiceTerms = $settings->invoice_terms_preset;
}
...
?>
    <div class="container" role="main">
        ...
        <div class="row">
            <div class="col-md-9 col-lg-9 col-sm-9 col-xs-6 vcenter"><h2><?php echo(!empty($$pt_id)?"Edit":"Add") ?> Invoice</h2></div><!--
            ...
            <span class="back_to_list">&larr;<a href="index.php">Back to list</a></span> </div>
                ...
                <div class="col-md-12 col-lg-11 col-sm-12 col-xs-12 ">
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <input type="text" class="form-control typeaheadCustomer" name="customerName" id="customerName" placeholder="" value="<?php echo $a->crl($customerName) ?>"
                            ...
                            if (!empty($$pt_id)) {
                                ...
                                foreach ($res->result_array() as $item) {
                                    ...
                                    <div class="row blue_section">
                                        <a class="removeIcon" href="javascript:;" onclick="deleteRow(this,<?php echo($item['idItem']) ?>)">
                                        ...
                                        <div class="col-md-10 col-sm-10 col-xs-9 form_left_section vcenter">
                                            <div class="form-group col-md-4 col-sm-12 col-xs-12">
                                                ...
                                                <input type="text" class="form-control itemName typeahead"
                                ...
                                <div class="row blue_section">
                                    <div class="col-md-10 col-sm-10 col-xs-9 form_left_section vcenter">
                                        <div class="form-group col-md-4 col-sm-12 col-xs-12">
                                            <label for="itemName"><span>*</span>Item</label>
                                            <input type="text" class="form-control itemName typeahead" name="itemName[]"
                                                   id="itemName_0" placeholder=""
                                                   value="" data-rule-required="true"
                                                   data-msg-required="Item name are required">
                                            <input type="hidden" name="itemItem[]" value="">
                                            <label>Description</label>
                                        ...
                                        <div class="form-group col-md-2 col-sm-12 col-xs-12">
                                            <label for="itemName"><span>*</span>Quantity</label>
                                            <input type="text" class="form-control itemQty" name="itemQty[]"
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            ...
                            <?php echo !empty($$pt_id) && $invoiceStatus == 'paid'?'type="button" onclick="confirmSave(\'save\')"':'type="submit"' ?>>
                            Save</button>
                        </div>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <button name="sendInvoice" class="btn btn-success btn-lg"
                                <?php echo !empty($$pt_id) && $invoiceStatus == 'paid'?'type="button" onclick="confirmSave(\'send\')"':'type="submit"' ?>
                            >Save &amp; Send
                            </button>
    ...
    <?php if(!empty($$pt_id)){?>
        ...
        <!-- Modal Invoice history-->
        <div class="modal fade" id="confirmEdit" tabindex="-1" role="dialog" aria-labelledby="confirmEdit">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" id="confirmEditLabel">Invoice History</h4>
                    </div>
                    <div class="modal-body">
                        You are saving changed to the invoice which was already paid by the customer. Please confirm that you want to save the changes.
                    </div>

                    <div class="modal-footer">
                        <button type="button" class="btn btn-success pull-left" onclick="$('#invoiceForm').submit()">Confirm</button>
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
