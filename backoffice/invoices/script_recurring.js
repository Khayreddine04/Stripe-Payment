...
$().ready(function () {
    ...
    $(".itemSelect").change(function () {
        ...
        var amount = $el.find('option:selected').data('amount');
        var period = $el.find('option:selected').data('period');
        console.log(amount);
        var rateInput = $el.parents(".blue_section").find("input[name^='itemRate']");
        if(!rateInput.length){
            var rateInput = $el.parents(".blue_section").find("input[name^='_itemRate']");
        }
        rateInput.val(amount);
        $el.parents('.form-group').find('textarea').val(period+' payment');
