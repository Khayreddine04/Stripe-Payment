...
function getCountryFromIP()
{
    ...
    if ($response === false) {
        ...
        if (strpos($http_response_header[0], '429') !== false) {
            error_log("IP Lookup - ipapi.co rate limited: " . $errorMessage);
        } else {
            error_log("IP Lookup - ipapi.co request failed: " . $errorMessage);
        }
    } else {
...
try {
    ...
    } elseif (!in_array($effectiveTheme, ['adaptive-lp', 'Minimalist', 'Colorful', 'PhysicalProduct']) && basename($_SERVER['PHP_SELF']) !== 'index.php') {
...
$payment_info_data = array_merge($c->post, [
    ...
    'post' => $c->post,
    'payment' => $payment,
    'detectedCountry' => $detectedCountry, // Pass detected country to template
    'settings' => $settings,
...
if ($pt_action == 'do_payment') {
    ...
    pt_checkout_timing_log('final_post_start', null, array('file' => 'index3.php'));
    ...
    if ($c->checkCaptcha()) {
        ...
        error_log("Payment method from POST: " . ($_POST['stripe_payment_method'] ?? 'NOT SET'));
        error_log("stripeToken from POST: " . ($_POST['stripeToken'] ?? 'NOT SET'));

        $paymentResult = $payment->doPayment();
        pt_checkout_timing_log('final_doPayment', $checkoutFinalTimingStart, array('file' => 'index3.php', 'res' => $paymentResult === true ? '1' : '0'));
        ...
        if ($paymentResult === true) {
            ...
            if ($pt_type != 'paypal') {
                if (!empty($settings->thank_you_redirect)) {
                    ...
                    pt_checkout_timing_log('final_redirect', $checkoutFinalTimingStart, array('file' => 'index3.php'));
                    header('Location: ' . $redirectUrl);
                    exit();
                } else {
                    $submit_data = array();
...
?>
    ...
    <div class="container main" role="main">
                    ...
                    <div class="form-columns">
                        ...
                        <div class="payment-details-column">
                            <form class="validate payment_form" role="form" id="payment_form" method="post">
                                ...
                                <input type="hidden" name="pt_tax_rate" id="pt_tax_rate" readonly value="<?php echo $settings->tax_enable == 'y' ? $settings->tax_rate : 0; ?>" />
                                ...
                                <input type="hidden" name="pt_service" value="<?php echo htmlspecialchars($_GET['service'] ?? '', ENT_QUOTES); ?>">
                                <input type="hidden" name="amount" value="<?php echo htmlspecialchars($_GET['amount'] ?? '', ENT_QUOTES); ?>">
                                <input type="hidden" name="pt_amount" value="<?php echo htmlspecialchars($pt_amount ?? '', ENT_QUOTES); ?>">
                                <input type="hidden" name="pt_currency" value="<?php echo htmlspecialchars($settings->currency_text ?? 'USD', ENT_QUOTES); ?>">
                                <input type="hidden" name="pt_currency_symbol" value="<?php echo htmlspecialchars($settings->display_currency ?? '$', ENT_QUOTES); ?>">
                                <input type="hidden" name="pt_currency_position" value="<?php echo htmlspecialchars($settings->currency_position ?? 'before', ENT_QUOTES); ?>">
                                ...
                                <input type="hidden" name="clickid" value="<?php echo htmlspecialchars($_GET['clickid'] ?? ($_GET['cid'] ?? ''), ENT_QUOTES); ?>">
                                <input type="hidden" name="source" value="<?php echo htmlspecialchars($_GET['source'] ?? '', ENT_QUOTES); ?>">
        ...
        <script src="assets/js/PhysicalProduct/payment_form.js?v=<?php echo rand(1, 9999) ?>" type="application/javascript"></script>
